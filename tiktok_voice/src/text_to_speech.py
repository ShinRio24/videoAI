...
def _fetch_audio_bytes(
    ...
    def generate_audio_chunk(index: int, text_chunk: str):
        try:
            response = requests.post(endpoint["url"], json={"text": text_chunk, "voice": voice.value, "speed": 2})
...
def _load_endpoints() -> List[Dict[str, str]]:
    ...
    json_file_path = os.path.abspath(os.path.join(os.path.dirname(__file__),  '..', 'data', 'config.json'))
